...
ObjectFileCOFF::ObjectFileCOFF(const ModuleSP &module_sp, DataBufferSP data_sp,
  ...
  if (binary)
    m_object = unique_dyn_cast<COFFObjectFile>(std::move(*binary));
  else
    LLDB_LOG_ERROR(log, binary.takeError(),
                   "Failed to create binary for file ({1}): {0}", m_file);

  LLDB_LOG(log,
           "{0} ObjectFileCOFF::ObjectFileCOFF module = {1} ({2}), file = {3}, "
           "object = {4}",
           this, GetModule().get(), GetModule()->GetSpecificationDescription(),
           m_file.GetPath(), m_object.get());
}
...
void ObjectFileCOFF::CreateSections(lldb_private::SectionList &sections) {
  ...
  auto SectionType = [](const object::coff_section *Section) -> lldb::SectionType {
    lldb::SectionType type =
        StringSwitch<lldb::SectionType>(Section->Name)
  ...
  for (const auto &SecRef : m_object->sections()) {
    ...
    SectionSP section =
        std::make_unique<Section>(module, this,
                                  ...
                                  ConstString(COFFSection->Name),
                                  SectionType(COFFSection),
