...
class BCLiteral : public detail::BCField<> {
public:
...
class BCFixed : public detail::BCField<> {
public:
...
/// Note that the LLVM bitcode format only supports unsigned values.
template <unsigned Width>
class BCVBR : public detail::BCField<> {
...
class BCArray : public detail::BCField<true> {
...
public:
  ...
  static constexpr bool value =
      !std::is_same<decltype(check((T*)nullptr)), decltype(check(false))>::value;
};
...
public:
  ...
  static void readRecord(ArrayRef<ElementTy> buffer, Data &&... data) {
    ...
    static_assert(sizeof...(Fields) <=
                  ...
                  "Too few record elements");
    return detail::BCRecordCoding<Fields...>::read(
