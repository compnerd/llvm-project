...
public:
  ...
  void emitUnversionedInfo(raw_ostream &OS, const ObjCPropertyInfo &OPI) {
    ...
    uint8_t flags = 0;
    if (Optional<bool> value = OPI.getSwiftImportAsAccessors()) {
      flags |= 1 << 0;
...
void emitParamInfo(raw_ostream &OS, const ParamInfo &PI) {
  ...
  if (auto RCC = PI.getRetainCountConvention())
    flags |= static_cast<uint8_t>(RCC.getValue()) + 1;
...
public:
  ...
  void emitUnversionedInfo(raw_ostream &OS, const TagInfo &TI) {
    ...
    if (auto extensibility = TI.EnumExtensibility) {
      Flags |= static_cast<uint8_t>(extensibility.getValue()) + 1;
      assert((Flags < (1 << 2)) && "must fit in two bits");
    }

    Flags <<= 2;
    if (llvm::Optional<bool> Value = TI.isFlagEnum())
      Flags |= (Value.getValue() << 1 | 1 << 0);
