...
namespace yaml {
template <>
struct ScalarEnumerationTraits<SwiftWrapperKind> {
  static void enumeration(IO &IO, SwiftWrapperKind &SWK) {
    IO.enumCase(SWK, "none",   SwiftWrapperKind::None);
    IO.enumCase(SWK, "struct", SwiftWrapperKind::Struct);
    IO.enumCase(SWK, "enum",   SwiftWrapperKind::Enum);
  }
};
...
struct ScalarTraits<VersionTuple> {
  static void output(const VersionTuple &VT, void *, raw_ostream &OS) {
    OS << VT;
  }
  static StringRef input(StringRef S, void *, VersionTuple &VT) {
    if (VT.tryParse(S))
